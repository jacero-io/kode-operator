...
func main() {
	...
	if err = (&controller.EnvoyProxyReconciler{
		...
		setupLog.Error(err, "unable to create controller", "controller", "EnvoyProxy")
		os.Exit(1)
	}
	if err = (&controller.EnvoyClusterReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "EnvoyCluster")
		os.Exit(1)
	}
